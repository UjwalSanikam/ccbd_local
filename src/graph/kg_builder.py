...
def load_fused_graph(fused_path: Path) -> nx.DiGraph:
    """
    Load the merged fused knowledge graph and convert it into the
    typed KG expected by hop_reasoner.
    """
    data = json.loads(fused_path.read_text(encoding="utf-8"))
    logger.info("Loading fused graph from %s", fused_path)
    ...
    logger.info("Loaded fused KG: %d nodes, %d edges", G.number_of_nodes(), G.number_of_edges())
    return G


def build_knowledge_graph(matches_path: Path) -> nx.DiGraph:
    """
    Main function. Reads either fused_knowledge_graph.json or entity_matches.json.

    If the input JSON contains a "links" or "edges" key, it is treated as a
    pre-built fused knowledge graph. Otherwise, it falls back to the legacy
    entity_matches input format.
    """
    data = json.loads(matches_path.read_text(encoding="utf-8"))
    if "links" in data or "edges" in data:
        return load_fused_graph(matches_path)
